...
- **강력한 개인정보 비식별화**:
  - 대화 내용에서 본명, 주소, 전화번호, 계좌번호, 주민등록번호 등의 개인정보는 철저하게 마스킹 처리하여 개인정보 보호
...
- **딥보이스 탐지**:
  ...
  - 실시간 탐지에 중점을 두기 때문에 `5초` 분량의 음성을 듣고 이상 유무를 판단하도록 `무음구간제거`, `5초 구간 내의 특징 추출`, `패딩 처리`, `랜덤 노이즈 증강`, `4kHz 고주파 제거` 등의 전처리 후 모델 학습에 사용
  ...
  - 최종적으로 딥보이스 탐지에는 총 2개의 분류기를 사용하며, 소프트 보팅 기법을 통해 최종 결과를 산출하여 딥보이스를 판단
...
- **통화 요약 리포트**:
  ...
  - `Persona` : 모델에게 보이스피싱 분류 전문가 페르소나를 부여하여 분석의 정확도 향상
  ...
  - 리포트에는 시각화 기법을 추가하여 사용자가 긴 통화 내용을 한 눈에 쉽게 파악할 수 있도록 설계
 
- **챗봇**:
  - 실제 보이스 피싱 대화를 임베딩하여 `VectorDB(FAISS)` 구축 후 적재
  - 통화 리포트 내용을 기반으로 주요 키워드를 추출하여 `Chip` 제공
  - `RAG` 기법을 사용하여 사용자의 질문과 관련된 유사한 통화 내역을 찾아 답변 제공(최대 10초 내로 답변 제공, 대기 중에 로딩바 출력)
  - 질문과 유사한 통화 내역이 없을 경우, 관련된 사례가 없다는 답변 출력
...
- **사후 관리**:
  - `FastAPI`와 조합이 좋은 RDBMS인 `PostgreSQL` 사용하여 피싱 키워드, 세이프 키워드, 신고 내역 등을 암호화하여 저장
  ...
  - 해당 키워드로 걸러진 대화 내용들을 따로 분류하여 학습용 데이터셋으로 구축
    - `AutoEnocder` Loss가 높은데 `KoBERT` 결과가 정상 : 정상(0)
    - `AutoEncoder` Loss가 낮은데 `koBERT` 결과가 피싱 : 피싱(1)
