...
- **대화 맥락 탐지**:
  - 일상 대화 약 750만개를 사용하여 `AutoEncoder` 모델 구축 후 학습이 되지 않은 비정상 대화가 입력될 경우, 탐지된 시점부터 최근 3개의 대화 내용을 맥락으로 구성하여 `koBERT` 모델에 전송
    - 단일 모델로 구성했을 때보다 `AutoEncoder`로 1차적인 필터링 과정을 거치는 것이 성능이 대폭 향상되어 사용하였음
    - 보이스피싱에 주로 사용되는 피싱 키워드를 추출하고, 가중치 부여
  - 한국어 사전 학습된 모델 `koBERT` 에 금융/상담 데이터와 실제 보이스피싱 대화 데이터를 사용하여 파인 튜닝을 진행하고, 대화 맥락 정밀 분석 진행(이진 분류)
  - 분석이 진행될 때마다 의심 스코어를 계산하고 누적이 되어 임계치를 초과할 경우 보이스피싱이라고 판단하여 실시간 경고 알림 전송

- **딥보이스 탐지**:
  - 음성에서 `Mel-Spectogram` 기법을 활용하여 음성의 특징을 추출하여 `CNN` 모델 구축 후 이진 분류
  - 추가적으로 음성에서 `MFCC`를 추출하고 `Res2Net-50-SE` 모델 구축 후 이진 분류
  ...
  - 실시간 탐지에 중점을 두기 때문에 `5초` 분량의 음성을 듣고 이상 유무를 판단하도록 `무음구간제거`, `5초 분량의 MFCC 추출`, `패딩 처리` 등의 전처리 후 모델 학습에 사용
  ...
  - 딥보이스 탐지에는 총 2개의 분류기를 사용하며, 소프트 보팅 기법을 통해 최종 결과를 산출하여 딥보이스를 판단

- **감성 분류**:
  - `Wav2Vec 2.0` 사전 학습 모델에 파인 튜닝 진행
...
- **챗봇**:
  - 실제 보이스 피싱 대화를 이용해 `VectorDB(FAISS)` 구축
  - `RAG` 기법을 사용해 질문과 관련된 통화 내역을 찾아 답변 제공
...
- **사후 관리**:
  - `FastAPI`와 가장 조합이 좋은 RDBMS인 `PostgreSQL` 사용하여 피싱 키워드, 세이프 키워드, 신고 내역 등을 암호화하여 저장
  - 새로운 피싱 수법 등장에 대비하여, 매일 자정에 키워드들은 자동 업데이트
...
|  |  |  |
|-----------------|-----------------|-----------------|
| 허성욱<br>[GitHub](https://github.com/dokpe01) | `팀장` | <ul><li> 프로젝트 총괄 및 일정 관리 </li> <li> 전체 파이프라인 설계 </li> <li> 대화 맥락 탐지 모델 개발 <li> 딥보이스 탐지 모델 개발 </li> <li> 감성 분류 모델 개발 </li> <li> 전체적인 모델 성능 개선 </li> </ul> |
| 이경민<br>[GitHub](https://github.com/bidulgirin) | `팀원` | <ul><li> 앱 개발 <li> 데이터베이스 설계 및 구축 </li> <li> FastAPI 서버 구축 및 배포 </li> <li> 사후 관리(유지 보수) </li> </ul> |
| 김나영<br> [GitHub](https://github.com/NA-young524) | `팀원` | <ul><li> 통화 요약 리포트 작성모델 개발 </li><li> 챗봇 개발 </li> <li> 딥보이스 탐지 모델 개발 </li> </ul> |
| 김정안<br>[GitHub](https://github.com/jeongan47) | `팀원` | <ul><li> 데이터 수집/분석/시각화/전처리 </li><li> 음성 데이터 전처리(STT) </li></ul> |
...
# 7. App Features (앱 기능) - 작성중
- 앱 완성 이후, 실제 구현된 스크린샷 업로드 예정
