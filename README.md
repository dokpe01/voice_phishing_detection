...
- 하지만 개인정보 유출 문제는 예전부터 지속적으로 발생하였고, 개인정보 유출이 보이스피싱 피해 증가와 상관관계가 있다고 보기는 어렵다.<br>
...
| 구분 | 특징 |
| :--: | :-- |
| `정상 음성` | 16kHz 대역까지 에너지가 살아있음(미세한 노이즈 등이 보존된 자연스러운 소리) <br> 고주파 대역에서도 분포 유지(주파수 불규칙, 풍부하게 변함) <br> 무음 구간에도 미세한 환경 노이즈가 포함됨|
| `변조 음성` | 에너지가 급격하게 감소한 뒤 완벽하게 0에 고정됨 <br> AI가 고주파 대역을 구현하지 못했거나 용량 효율을 위해 자동으로 0을 채워넣음 <br> 무음 구간에서 "완벽한 무음 상태"를 보임 |
...
| 구분 | 특징 |
| :--: | :-- |
| `기관사칭형` | 권위를 사칭하고 어떠한 사건을 제시하여 피해자를 불안하게 만들며 시작 <br> 권위 사칭을 이용해 피해자의 추가적인 정보를 확보 <br> 추가적인 통제 및 압박/추가 지시를 통해 송금 유도 |
| `대출사기형` | 피해자가 혹할만한 조건을 제시 <br> 추가 지시를 통해 개인정보 확보 <br> 특정 핑계를 대며 피해자를 통제 <br> 송금 유도(좋은 조건의 신규 대출을 위해 기존 대출금 상환 등) |
...
- **대화 맥락 탐지**:
  - TF-IDF 벡터라이저(등장 빈도 가중치 벡터)와 문맥 기반의 파인 튜닝을 통해 보이스피싱 특유의 키워드와 어휘 패턴을 학습
  - 일상 대화 약 750만개를 사용하여 `AutoEncoder` 모델 구축 후 학습이 되지 않은 비정상 대화가 입력될 경우, 탐지된 시점부터 최근 3개의 대화 문장을 연결자로 이어붙힌 맥락으로 구성하여 `koBERT` 모델에 전송
    - Enocder에서 입력된 문장의 크기를 점차 줄여나가며 중요하지 않은 수식어(불용어 등)를 제거하여 문장의 핵심 특징만을 남기고, Decoder에서 원래의 문장을 다시 복원시킴
    - 단일 모델로 구성했을 때보다 `AutoEncoder`로 1차적인 필터링 과정을 거치는 것이 성능 대폭 향상
      - 통화의 대다수를 차지하는 일상적인 대화는 `AutoEncoder`가 처리하므로 연산량/서버 비용 대폭 감소
      - 사용자 경험 개선(분석 결과 제공 속도 향상)
  ...
  - 한국어 사전 학습된 모델 `koBERT` 에 금융/상담 데이터와 실제 보이스피싱 대화 데이터를 사용하여 파인 튜닝을 진행하고, 대화 맥락 정밀 분석 진행(이진 분류)
    - `kOBERT`는 단어를 더 작은 단위로 쪼개어 처음보는 단어라도 의미가 있는 조각들로 나누어 분석이 가능
    - 문장 전체를 한꺼번에 보고 각 단어의 의미를 파악(문장 빈칸 맞추기)
    - Self-Attention : 문장 속의 모든 단어를 서로 연결해보고 어떤 단어가 현재 단어를 이해하는 데 중요한지 판단
  - 분석이 진행될 때마다 위험 스코어를 계산하고 누적이 되어 임계치를 초과할 경우 보이스피싱이라고 판단하여 실시간 경고 알림 전송
 
<hr>

| 구분 | CNN | Transformer | Res2Net |
| :--: | :-- | :-- | :-- |
| 기술 | 정해진 크기의 필터 내에서 특징 추출 | 음성의 모든 프레임이 서로 어떤 관계를 맺고 있는 지 계산 <br> 멀리 떨어진 구간의 정보도 동시에 고려 | 하나의 계층 안에서 데이터를 여러 그룹으로 분할 <br> 이를 계층적으로 연결 |
| 분석 | 특정 지역에서 발생하는 변조 흔적 포착 | 문장 전체의 억양, 호흡의 일관성 등 전체적인 맥락을 분석 | 다양한 시간/주파수 스케일을 동시에 분석 |
| 장점 | 음성의 지역적인 특징을 잡아내는 능력이 탁월 | 변조 음성의 미세한 흔적까지 잡아내어 정교한 탐지 가능 | 모델 크기 대비 성능이 매우 뛰어남 <br> 다양한 크기의 특징을 골고루 학습 <br> 과적합에 매우 강함 |
| 한계 | 특정 환경에 과적합 발생 <br> 새로운 환경의 음성을 만나면 성능이 급격히 감소 | 구조가 복잡하고 연산량이 많음 <br> 데이터 양이 적으면 초기 학습에 어려움 <br> 다양한 도메인에 적용할 때 성능 지표 모호 | - |
...
- **딥보이스 탐지**:
  ...
  - 최종적으로 딥보이스 탐지에는 총 2개의 분류기를 사용하며, 각 예측값을 입력으로 받는 `랜덤포레스트` 모델을 두어 `스태킹 앙상블` 기법을 통해 최종 결과 산출
...
<div>
  <img src="./screenshot/img/이탈율.png" width=700 height=auto/>
</div>
...
# 6. TEAM Tasks & Responsibilities (팀원 소개 및 역할 분담)
|  |  |  |
|-----------------|-----------------|-----------------|
